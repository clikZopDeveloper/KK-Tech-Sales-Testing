...
import com.example.kktext_testing.Adapter.DealerProdListAdapter
import com.example.kktext_testing.Adapter.ProductListAdapter
...
import com.example.kktext_testing.Utills.GeneralUtilities.getPath
import com.example.kktext_testing.databinding.ActivityAddLeadBinding
...
class AddLeadActivity : AppCompatActivity(), ApiResponseListner,
    ...
    var multipleProdlist: MutableList<MultipleProListBean> = ArrayList()
...
//binding.loginViewModel=loginViewModel
    ...
    fun apiAllGet() {
        ...
        apiClient.getApiPostCall(ApiContants.getBusinessCategory, params)
    }
    ...
    fun custTypeMode() {
        binding.rgCustType.setOnCheckedChangeListener(object : RadioGroup.OnCheckedChangeListener {
            override fun onCheckedChanged(group: RadioGroup?, checkedId: Int) {
    ...
    fun apiInsertLead() {
        ...
        builder.addFormDataPart("customer_type", custType)
        builder.addFormDataPart("business_name", businessCatName)
        builder.addFormDataPart("dealer_id",dealerID.toString())
    ...
    fun apiProductCategory(businessName: String) {
        SalesApp.isAddAccessToken = true
        apiClient = ApiController(activity, this)
        val params = Utility.getParmMap()
        params["business_category"] = businessName
        apiClient.progressView.showLoader()
        apiClient.getApiPostCall(ApiContants.getProductCategory, params)

    }

    fun apiProductSubCat(cat: String) {
       // Toast.makeText(this@AddLeadActivity,"test",Toast.LENGTH_SHORT).show()
        SalesApp.isAddAccessToken = true
        apiClient = ApiController(activity, this)
        val params = Utility.getParmMap()
    ...
    fun apiDealerProduct(dealerID: Int) {
        SalesApp.isAddAccessToken = true
        apiClient = ApiController(activity, this)
        val params = Utility.getParmMap()
        params["dealer_id"] = dealerID.toString()
        apiClient.progressView.showLoader()
        apiClient.getApiPostCall(ApiContants.GetDealerProduct, params)
    }
    ...
    override fun success(tag: String?, jsonElement: JsonElement) {
        try {
            ...
            if (tag == ApiContants.GetDealerProduct) {
                val dealerProductBean = apiClient.getConvertIntoModel<GetDealerProductBean>(
                    jsonElement.toString(),
                    GetDealerProductBean::class.java
                )
                if (dealerProductBean.error == false) {

                      handleDealerProdList(dealerProductBean.data)
                    /* SalesApp.mepList.clear()
                     SalesApp.mepList.addAll(mepBean.data)*/
                }else{
                    Toast.makeText(this, dealerProductBean.msg, Toast.LENGTH_SHORT).show()
                }
...
*/
            ...
            if (tag == ApiContants.getBusinessCategory) {
                val businessCatBean = apiClient.getConvertIntoModel<BusinessCatBean>(
                    jsonElement.toString(),
                    BusinessCatBean::class.java
                )
                if (businessCatBean.error == false) {
                    val state = arrayOfNulls<String>(businessCatBean.data.size)
                    for (i in businessCatBean.data.indices) {
                        //Storing names to string array
                        state[i] = businessCatBean.data.get(i).name
                    }

                    binding.stateBusinessCat.setAdapter(
                        ArrayAdapter(
                            this@AddLeadActivity,
                            android.R.layout.simple_list_item_1, state
                        )
                    ...
                    binding.stateBusinessCat.setOnItemClickListener(AdapterView.OnItemClickListener { parent, view, position, id ->
                        binding.stateBusinessCat.setText(parent.getItemAtPosition(position).toString())
                        businessCatName = parent.getItemAtPosition(position).toString()
                       /* for (projectCatBean in businessCatBean.data) {
                            if (projectCatBean.name.equals(
                                    parent.getItemAtPosition(position).toString()
                                )
                            ) {
                                businessCatName = projectCatBean.name
                                Log.d("StateID", "" + projectCatBean.id)
                            }
                        }*/
                        Toast.makeText(
                            applicationContext,
                            binding.stateBusinessCat.getText().toString(),
                            Toast.LENGTH_SHORT
                        ).show()
                        apiProductCategory(businessCatName)
                       // apiCatory(projectType)
                       // apiSubCatory(projectCatID)
                    })
                }
            ...
            if (tag == ApiContants.getProductSubCategory) {
                ...
                if (subProductCatBean.error == false) {
                    ...
                    for (i in subProductCatBean.data.indices) {
                        //Storing names to string array
                        state[i] = subProductCatBean.data.get(i).subCategory
                    }
                    ...
                    binding.productSubCategory.setOnItemClickListener(AdapterView.OnItemClickListener { parent, view, position, id ->
                        ...
                        for (productSubCatBean in subProductCatBean.data) {
                            if (productSubCatBean.subCategory.equals(
                                    parent.getItemAtPosition(position).toString()
                                )
                            ) {
                                subProdCatName = productSubCatBean.subCategory
                                Log.d("StateID", "" + productSubCatBean.subCategory)
                            }
                        }
                        ...
                        apiProductSubCat(productCategoryName)
                    })
                }
            }
...
//                    SalesApp.installerList.addAll(istallerBean.data)
            ...
            if (tag == ApiContants.getProductCategory) {
                ...
                if (productCategoryBean.error == false) {
                    setProductCat(productCategoryBean.data)
                }
            }
        } catch (e: Exception) {
    ...
    fun setProductCat(data: List<ProductCategoryBean.Data>) {
        val state = arrayOfNulls<String>(data.size)
        for (i in data.indices) {
            state[i] = data.get(i).category
        }
        ...
        binding.stateProductCategory.setOnItemClickListener(AdapterView.OnItemClickListener { parent, view, position, id ->
            ...
            for (productCatBean in data) {
                if (productCatBean.category.equals(parent.getItemAtPosition(position).toString())) {
                    productCategoryName = productCatBean.category
                    Log.d("StateID", "" + productCatBean.category)
                }
            }
    ...
    fun handleDealerProdList(data: List<GetDealerProductBean.Data>) {
        binding.rcDealerProduct.layoutManager = LinearLayoutManager(this)
        var mAdapter = DealerProdListAdapter(this, data, object :
            RvDealerClickListner {
            override fun clickPos(status: MutableList<MultipleProListBean>, id: Int) {
                multipleProdlist  =status
                Log.d("zxvzxv",Gson().toJson(multipleProdlist))
            }

        })
        binding.rcDealerProduct.adapter = mAdapter
        // rvMyAcFiled.isNestedScrollingEnabled = false
