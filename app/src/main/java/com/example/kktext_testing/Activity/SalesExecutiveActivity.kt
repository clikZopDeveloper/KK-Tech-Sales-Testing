...
import com.example.kktext_testing.databinding.ActivitySalesExecutiveBinding
    ...
    fun setClient(data: List<ExecutiveListBean.Data>) {
        ...
        binding.stateClient.setOnItemClickListener(AdapterView.OnItemClickListener { parent, view, position, id ->
            ...
            for (clientBean in data) {
                if (clientBean.userName.equals(parent.getItemAtPosition(position).toString())) {
                    clientID = clientBean.userId
                    Log.d("StateID", "" + clientBean.userId)
